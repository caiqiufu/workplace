...
public class QueryPushMessage extends SoapMessageHandler implements BizHandler {
	...
	public ProcessResult process(RequestInfo requestInfo, String parameters, Map<String, Object> extParameters)
		...
		List<AppPushMessage> datas = pushMessageBO.getPushMessage(serviceNumber, imei);
		JSONArray noteList = new JSONArray();
		if(datas!=null &&datas.size()>0){
			for (int i = 0; i < datas.size(); i++) {
				AppPushMessage appPushMessage = datas.get(i);
				JSONObject nodeDetails = new JSONObject();
				nodeDetails.put("subject", appPushMessage.getSubject());
				nodeDetails.put("text", appPushMessage.getText());
				nodeDetails.put("hyperlink", appPushMessage.getHyperlink());
