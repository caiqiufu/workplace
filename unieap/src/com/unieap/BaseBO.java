...
public abstract class BaseBO{
	...
	public void getPaginationDataByDetachedCriteria(DetachedCriteria detachedCriteria,Projection projection,PaginationSupport ps){
		...
		ps.setItems((List<Object>) DBManager.getHT(ps.getDsName()).findByCriteria(detachedCriteria, ps.getStartIndex(),ps.getPageSize()));
	}
	public void getPaginationDataByDetachedCriteria(DetachedCriteria detachedCriteria,Projection projection,PaginationSupport ps,Class<Object> vo, Class<?> po) throws InstantiationException, IllegalAccessException{
		int totalCount = getCountByDetachedCriteria(detachedCriteria,projection,ps.getDsName());
		ps.setTotalCount(totalCount);
		List<Object> datas = DBManager.getHT(ps.getDsName()).findByCriteria(detachedCriteria, ps.getStartIndex(),ps.getPageSize());
		if(datas!=null & datas.size()>0){
			List<Class<Object>> vdatas = new ArrayList<Class<Object>>();
	...
	public void getPaginationDataByDetachedCriteria(DetachedCriteria detachedCriteria,PaginationSupport ps){
		...
		List<Object> datas = (List<Object>) DBManager.getHT(ps.getDsName()).findByCriteria(detachedCriteria,ps.getStartIndex(),ps.getPageSize());
		ps.setItems(datas);
	}
	/**
	 * 针对sql预计，只参与分页数据组装
