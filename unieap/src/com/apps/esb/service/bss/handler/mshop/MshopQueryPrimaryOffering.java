...
@Service("mshopQueryPrimaryOffering")
public class MshopQueryPrimaryOffering implements BizHandler{
	...
	public ProcessResult process(RequestInfo requestInfo, Map<String, String> handler,
		...
		List<OfferingVO> primaryOfferingList = queryAvailableOfferingVO.getPrimaryOfferingList();
		if(handler!=null){
			...
			if(!StringUtils.isEmpty(custHandlerName)){
				...
				custHandler.process(requestInfo, handler, extParameters, primaryOfferingList,originalVOs);
			}
		}
		...
		jsonResult.put("offeringList",JSONUtils.collectionToJson(primaryOfferingList));
		return processResult;
	}
