...
@Service("queryCDR_1")
public class SmartQueryCDR extends CustSoapMessageHandler implements BizHandler {
	...
	public ProcessResult queryCDRSummary(String serviceNumber, RequestInfo requestInfo) throws Exception {
		...
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO1.getCdrInfoList());
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO2.getCdrInfoList());
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO3.getCdrInfoList());
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO4.getCdrInfoList());
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO5.getCdrInfoList());
		queryCdrVO.getCdrInfoList().addAll(queryCdrVO6.getCdrInfoList());
	...
	public ProcessResult queryCDRDetail(String serviceNumber, RequestInfo requestInfo) throws Exception {
		...
		ProcessResult processResult = process(sOAPMessage, requestInfo, requestInfo.getRequestBody().getServiceNumber(),
				requestInfo.getRequestBody().getExtParameters(), null, "ws.cbs.query.timeout");
		return processResult;
	}

	public SOAPMessage getRequestCDRDetailSOAPMessage(String totalCDRNum, String pageNum, String serviceNumber,
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				for (int j = 0; j < cDRSummaryNodes.getLength(); j++) {
					...
					if ("bbs:ServiceCategory".equals(cDRSummaryNode.getNodeName())) {
						...
						cdrSummaryVO.setServiceCategoryDesc(
