...
@Service("queryDataBalance_1")
public class SmartQueryDataBalance extends CustSoapMessageHandler implements BizHandler{
	...
	public ProcessResult getResposeResult(SOAPMessage response) throws Exception {
		...
		if (document.getElementsByTagName("bbs:FreeUnitItem").getLength() > 0){
			...
			for(int i = 0 ; i < freeUnitItemNodes.getLength() ; i++){
				...
				for(int j = 0 ; j < nodes.getLength() ; j++){
					...
					if ("bbs:FreeUnitType".equals(node.getNodeName())){
						freeResourceVO.setFreeResourceType(node.getTextContent());
					}else if("bbs:FreeUnitTypeName".equals(node.getNodeName())){
						freeResourceVO.setFreeResourceTypeDesc(node.getTextContent());
			...
			List<FreeResourceVO> newFreeResourceList = new ArrayList<FreeResourceVO>();
			for(int i = 0 ; i < freeResourceList.size() ; i++){
				FreeResourceVO freeResourceVO = freeResourceList.get(i);
				if("1108".equals(freeResourceVO.getMeasureUnit())){
					newFreeResourceList.add(freeResourceVO);
				}
			}
			queryFreeResourceVO.setFreeResourceList(newFreeResourceList);
		}
		return result;
	}
	@Override
