...
@Service("queryCorporateGroup_1")
public class SmartGetCorpGroupData extends CustSoapMessageHandler implements BizHandler {
	...
	public ProcessResult process(RequestInfo requestInfo, Map<String, String> handler, Map<String, Object> extParameters)
		...
		newRequestInfo.getRequestHeader().setBizCode("queryCorporateGroup");
	...
	public SOAPMessage getRequestSOAPMessage(String serviceNumber, RequestInfo requestInfo) throws Exception {
		...
		if (!json.has("customerId")) {
			throw new Exception("corporate customerId is null");
		}
		SOAPMessage message = SoapCallUtils.getMessageFactory().createMessage();
		this.getCRMQuerHeader("GetGroupRequest", message);
		SOAPBodyElement bodyElement = (SOAPBodyElement) message.getSOAPBody().getChildElements().next();
		SOAPElement getGroupBodyElement = bodyElement.addChildElement("GetGroupBody", "quer");
		getGroupBodyElement.addChildElement("CustomerId", "quer").addTextNode(json.getString("customerId"));
		if(json.has("groupId")){
			getGroupBodyElement.addChildElement("GroupId", "quer").addTextNode(json.getString("groupId"));
		}
		return message;
	}
