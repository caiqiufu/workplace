...
@Service("queryCorporateCustomer_1")
public class SmartGetCorpCustomerData extends CustSoapMessageHandler implements BizHandler {
	...
	public ProcessResult process(RequestInfo requestInfo, Map<String, String> handler, Map<String, Object> extParameters)
		...
		newRequestInfo.getRequestHeader().setBizCode("queryCorporateCustomer");
	...
	public SOAPMessage getRequestSOAPMessage(String serviceNumber, RequestInfo requestInfo) throws Exception {
		...
		if (!json.has("memberServiceNumber")&&!json.has("contactServiceNumber")) {
			throw new Exception("memberServiceNumber and contactServiceNumber one is mandatory");
		}
		
		SOAPMessage message = SoapCallUtils.getMessageFactory().createMessage();
		this.getCRMQuerHeader("GetCorpCustomerDataRequest", message);
		SOAPBodyElement bodyElement = (SOAPBodyElement) message.getSOAPBody().getChildElements().next();
		SOAPElement requestGetCustomerBodyElement = bodyElement.addChildElement("GetCorpCustomerBody", "quer");
		if(json.has("memberServiceNumber")){
			requestGetCustomerBodyElement.addChildElement("MemberServiceNumber", "quer").addTextNode(json.getString("memberServiceNumber"));
		}
		if(json.has("contactServiceNumber")){
			requestGetCustomerBodyElement.addChildElement("ContactServiceNumber", "quer").addTextNode(json.getString("contactServiceNumber"));
		}
		return message;
	}
