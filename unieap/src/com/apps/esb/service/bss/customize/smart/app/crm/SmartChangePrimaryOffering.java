...
@Service("changePrimaryOffering_1")
public class SmartChangePrimaryOffering extends CustSoapMessageHandler implements BizHandler {
	...
	public SOAPMessage getRequestSOAPMessage(String serviceNumber, RequestInfo requestInfo) throws Exception {
		...
		/**
		 * 1 add, 2 modify,3 delete, 4 upgrade , 5 downgrade, 9 migrate , 10
		 * continue(reserved)
		 */
		...
		if (offeringInfo.has("supplementaryOfferings")) {
			...
			if (supplementaryOfferings != null && supplementaryOfferings.length() > 0) {
				SOAPElement supplementaryOfferingListElement = subscriberElement
						.addChildElement("SupplementaryOfferingList", "bas");
				for (int i = 0; i < supplementaryOfferings.length(); i++) {
					...
					SOAPElement subOfferingInfoElement = supplementaryOfferingListElement
							.addChildElement("OfferingInfo", "bas");
