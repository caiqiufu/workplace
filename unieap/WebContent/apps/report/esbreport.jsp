...
<title>complain management</title>
    ...
    Ext.onReady(function(){
    	...
    	var gridstoreForNote = Ext.create('Ext.data.Store', {
            ...
            pageSize: <%=SYSConfig.getConfig().get("pageSize")%>,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=messageGrid&type=B',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
    	...
    	var gridstoreForPopup = Ext.create('Ext.data.Store', {
            ...
            pageSize: <%=SYSConfig.getConfig().get("pageSize")%>,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=messageGrid&type=P',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
    	...
    	var gridstoreForRound = Ext.create('Ext.data.Store', {
            ...
            pageSize: <%=SYSConfig.getConfig().get("pageSize")%>,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=groupGrid&groupNames=top_round_displays',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
    	...
    	var gridstoreForAd = Ext.create('Ext.data.Store', {
            ...
            pageSize: <%=SYSConfig.getConfig().get("pageSize")%>,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=groupGrid&names=app_ad_page,chongzhi_ad',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
    	...
    	var gridstoreForDeno = Ext.create('Ext.data.Store', {
            ...
            pageSize: <%=SYSConfig.getConfig().get("pageSize")%>,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=denoGrid',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
	        	...
	        	[
					{
					    ...
					    title: '<%=UnieapConstants.getMessage("report.mcare.install.title")%>',
					    items : [
									gridForNotification,
									{
									    region: 'east',
					    ...
					    xtype: 'panel',layout: 'border',height:530,
					    title: '<%=UnieapConstants.getMessage("report.mcare.bizprocess.title")%>',
					    items : [
									gridForNote,
									{
									    region: 'east',
