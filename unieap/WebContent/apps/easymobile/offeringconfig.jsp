...
<title>User</title>
    ...
    Ext.onReady(function(){
    	...
    	var gridstore = Ext.create('Ext.data.Store', {
            ...
            pageSize: 10,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=offerCategoryGrid',
                reader: 
	          	...
	          	listeners:{
			        	...
			        	if(UnieapButton.Offering_Category_Add!=null&&UnieapButton.Offering_Category_Add.abled== true){
       	   	   	...
       	   	   	[
       	   	  		...
       	   	  		{ text: "<%=UnieapConstants.getMessage("mcare.offering.display.priceDesc")%>", dataIndex: 'priceDesc', sortable: false,width:100},
       	   	  		{ text: "<%=UnieapConstants.getMessage("comm.hyperlink")%>", dataIndex: 'detailHyperlink', sortable: false,width:120,
        ...
        function submitCategory(form){
        	...
        	if (form.isValid()){
                 form.submit({
                     ...
                     url: 'easyMobileController.do?method=offerCategoryDeal&operType='+operType+'&parameters='+encodeURL(parameters),
    	...
    	var offeringGridStore = Ext.create('Ext.data.Store', {
            ...
            pageSize: 15,
            remoteSort: true,
            proxy:{ type: 'ajax', url: 'easyMobileController.do?method=offeringGrid',
                reader: 
                {root: 'rows', totalProperty: 'totalCount'},
                simpleSortMode: true
            },
		                        	...
		                        	var form = dataFormOffering.getForm();
		                        	 if (form.isValid()){
		                                 form.submit({
		                                     ...
		                                     waitMsg: '<%=UnieapConstants.getMessage("comm.processing")%>',
		                                     url: 'easyMobileController.do?method=offeringDeal',
		                                     success: function(form, action) {
		                                    	var result = Ext.JSON.decode(action.response.responseText);
							                    if(result.isSuccess == 'failed'){
							                    	Ext.MessageBox.show({title: '<%=UnieapConstants.getMessage("comm.status")%>',msg:result.message,
