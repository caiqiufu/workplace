	        ...
	        tbar:
	        	[
					...
					{ xtype:'button',id:"role_save",iconCls :'save',text:'<%=UnieapConstants.getMessage("comm.submit")%>',iconAlign: 'left',pressed :true,
					    handler: function (){
					    	Ext.MessageBox.confirm('<%=UnieapConstants.getMessage("comm.title.confirm")%>', '<%=UnieapConstants.getMessage("comm.confirm.save")%>', saveInfoRole);
					    }
					}
	             ]
	    });
